...
int tx_build() {
    ...
    /* output 1 (0.251 BTC) to send */
    bbp_txout_create_p2pkh(&outs[0], 25100000, "18ba14b3682295cb05230e31fecb000892406608");

    /* output 2 (change to get back, 0.619 BTC) */
    bbp_txout_create_p2pkh(&outs[1], 61900000, "6bf19e55f94d986b4640c154d864699341919511");

    /* input from utxo (0.87 BTC) */
    bbp_outpoint_fill(&outpoint, "f34e1c37e736727770fed85d1b129713ef7f300304498c31c833985f487fa2f3", 0); // The UTXO to spend from
    bbp_txout_create_p2pkh(&prev_outs[0], 87000000, "6bf19e55f94d986b4640c154d864699341919511");
    bbp_txin_create_signable(&ins_sign[0], &outpoint, &prev_outs[0]); // Set input script to UTXO script to make it signable
